...
class StateMachine():
    ...
    def update_stop_go_count(frame_index):
        ...
        last_state = StateMachine.sg_queue[-1][0] if StateMachine.sg_queue else None
        frame = frames[frame_index]
        ...
        if last_state == "ACCEL" and brake > 1:
            StateMachine.sg_count += 1
            StateMachine.sg_queue.append(("BRAKE", timestamp))
        elif last_state == "BRAKE" and throttle > 1:
            StateMachine.sg_count += 1
            StateMachine.sg_queue.append(("ACCEL", timestamp))
        elif not last_state:
            last_state = "ACCEL" if throttle > 1 else "BRAKE"
            StateMachine.sg_count += 1
            StateMachine.sg_queue.append((last_state, timestamp))

        while StateMachine.sg_queue:
            _, time = StateMachine.sg_queue[0]
            if timestamp - time >= 60:
                StateMachine.sg_queue.pop(0)
                StateMachine.sg_count -= 1
            else:
                break
    ...
    def is_right_turn(frame_index):
        ...
        return steering_angle < StateMachine.RIGHT_TURN_THRESHOLD and (left_wheel_speed - right_wheel_speed) >= 1.0
    ...
    def is_left_turn(frame_index):
        ...
        right_wheel_speed = frame.vehicle_wheel_speeds.front_right
        steering_angle = frame.steering_ang.steering_wheel_angle
        return steering_angle > StateMachine.LEFT_TURN_THRESHOLD and (right_wheel_speed - left_wheel_speed) >= 1.0
    ...
    def update_traffic_light_behavior(frame_index):
        ...
        """
        frame = frames[frame_index]
        if not StateMachine.update_yellow_detect(frame):
            return 0

        throttle = frame.accelerator_pedal.throttle_rate
        brake = frame.brake_torq.brake_torque_request
        StateMachine.throttle_queue.append(throttle)
        StateMachine.brake_queue.append(brake)
        avg_throttle = sum(StateMachine.throttle_queue[1:])/(len(StateMachine.throttle_queue)-1) if len(StateMachine.throttle_queue) > 1 else 0
        avg_brake = sum(StateMachine.brake_queue[1:])/(len(StateMachine.brake_queue)-1) if len(StateMachine.brake_queue) > 1 else 0
        # Once we have enough frames to determine behavior
        if len(StateMachine.throttle_queue) >= StateMachine.SPEED_UP_TIME / StateMachine.MASTER_CSV_INTERVAL:
            # Default behavior: Slow down upon seeing the yellow light. Expected, cautious.
            if avg_brake > StateMachine.brake_queue[0] and avg_throttle <= StateMachine.throttle_queue[0]:
                StateMachine.brake_queue = []
                StateMachine.throttle_queue = []
                return 0
            # If we speed up after seeing the yellow light, reckless
            elif avg_throttle > StateMachine.throttle_queue[0] and avg_brake <= StateMachine.brake_queue[0]:
                StateMachine.brake_queue = []
                StateMachine.throttle_queue = []
                return 1
            else:
    ...
    def video_joiner(folder_path):
        names = StateMachine.list_files(folder_path)
...
interpreter = Interpreter(statechart, initial_context={'s': StateMachine})
