...
def print_help():
    ...
    print
    print 'converts image sequence(s) in ros bag file(s) to video file(s) with fixed frame rate using ffmpeg'
    print 'ffmpeg needs to be installed! (sudo apt-get install libav-tools)'
    ...
    print 'ffmpeg will guess the format according to given extension'
            ...
            if msg.format.find("jpeg")!=-1 :
                ...
                if len(msg.data)>0:
                    ...
                    while t_video[topic]<t_file[topic]:
                        if not topic in p_ffmpeg:
                            if opt_out_file=="":
                                out_file = str(topic).replace("/", "")+".mp4"
                            else:
                                out_file = opt_out_file
                            p_ffmpeg[topic] = subprocess.Popen(['ffmpeg','-r',str(opt_fps),'-an','-c','mjpeg','-f','mjpeg','-i','-',out_file],stdin=subprocess.PIPE)
                        p_ffmpeg[topic].stdin.write(msg.data)
                    ...
                    if len(msg.data)>0:
                        ...
                        while t_video[topic]<t_file[topic]:
                            if not topic in p_ffmpeg:
                                ...
                                p_ffmpeg[topic] = subprocess.Popen(['ffmpeg','-r',str(opt_fps),'-an','-f','rawvideo','-s',size,'-pix_fmt', pix_fmt,'-i','-',out_file],stdin=subprocess.PIPE)
                            p_ffmpeg[topic].stdin.write(msg.data)
                            t_video[topic] += 1.0/opt_fps
                        if opt_display_images:
                            cv2.imshow(topic, cv_image)
